...
class Dataset:
    ...
    def __init__(self,
        ...
        if self.dataset_name == "cifar10":
            ...
            print("Test set length: ", len(self.test_set))
    ...
    def set_target(self, key, target):
        ...
        elif self.dataset_name == "tissue" or self.dataset_name == "chest":
    ...
    def compute_noise_transition_instance(self):
        ...
        for i, (image, label, _, _) in enumerate(self.train_set):
            # 1*m *  m*10 = 1*10 = A.size()
            image = image.detach().to(self.device)
            A = image.reshape(1, -1).mm(W[label]).squeeze(0)
            A[label] = -inf
